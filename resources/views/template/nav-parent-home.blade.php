    ...
    <ul class="flex gap-8 text-gray-400">
        <li class="flex flex-col items-end">
            ...
            <div class="relative inline-block text-left">
                <div>
                    <button type="button"
                        class="transition-all inline-flex w-full justify-center gap-x-1.5 rounded-md bg-white px-3 py-2 text-sm font-medium text-gray-900  ring-gray-300 hover:bg-gray-50"
                ...
                <div id="child-dropdown"
                    ...
                    <div class="py-1" role="none">
                        @foreach ($students as $student)
                            <a href="/{{ $page }}?student={{ $student->id }}"
                                class="text-gray-700 block px-4 py-2 text-sm hover:bg-slate-100 transition-all" role="menuitem"
                                tabindex="-1" id="menu-item-0">{{ $student->name }}</a>
                        @endforeach
                    </div>
                </div>
            </div>
        ...
        <script type="text/javascript">
            document.getElementById('menu-button').addEventListener('click', function() {
                const menu = document.getElementById('child-dropdown');
